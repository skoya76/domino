...
var logger = logging.MustGetLogger("epaxos")

var isUseBeacon bool = false // Added by xy for fixing Epaxos beacon bug
...
func NewReplica(id int, peerAddrList []string, thrifty bool, exec bool, dreply bool, beacon bool, durable bool,
	...
	r.Beacon = beacon
	r.Durable = durable
	isUseBeacon = beacon // Added by xy to fix EPaxos beacon bug
	...
	logger.Infof("Epaxos thrifty = %t, beacon = %t, exec = %t, dreply = %t", r.Thrifty, r.Beacon, r.Exec, r.Dreply)
...
func (r *Replica) run() {
	...
	for !r.Shutdown {
		...
		case propose := <-onOffProposeChan:
			...
			//logger.Infof("Proposal with op %d\n", propose.Command.Op)
		...
		case prepareS := <-r.prepareChan:
			...
			//logger.Infof("Received Prepare for instance %d.%d", prepare.Replica, prepare.Instance)
		...
		case preAcceptS := <-r.preAcceptChan:
			...
			//logger.Infof("Received PreAccept for instance %d.%d", preAccept.LeaderId, preAccept.Instance)
		...
		case acceptS := <-r.acceptChan:
			...
			//logger.Infof("Received Accept for instance %d.%d", accept.LeaderId, accept.Instance)
		...
		case commitS := <-r.commitChan:
			...
			//logger.Infof("Received Commit for instance %d.%d", commit.LeaderId, commit.Instance)
		...
		case prepareReplyS := <-r.prepareReplyChan:
			...
			//got a Prepare reply
			//logger.Infof("Received PrepareReply for instance %d.%d", prepareReply.Replica, prepareReply.Instance)
		...
		case preAcceptReplyS := <-r.preAcceptReplyChan:
			...
			//logger.Infof("Received PreAcceptReply for instance %d.%d", preAcceptReply.Replica, preAcceptReply.Instance)
		...
		case preAcceptOKS := <-r.preAcceptOKChan:
			...
			//got a PreAccept reply
			//logger.Infof("Received PreAcceptOK for instance %d.%d", r.Id, preAcceptOK.Instance)
		...
		case acceptReplyS := <-r.acceptReplyChan:
			...
			//got an Accept reply
			//logger.Infof("Received AcceptReply for instance %d.%d", acceptReply.Replica, acceptReply.Instance)
		...
		case tryPreAcceptS := <-r.tryPreAcceptChan:
			...
			//logger.Infof("Received TryPreAccept for instance %d.%d", tryPreAccept.Replica, tryPreAccept.Instance)
		...
		case tryPreAcceptReplyS := <-r.tryPreAcceptReplyChan:
			...
			//logger.Infof("Received TryPreAcceptReply for instance %d.%d", tryPreAcceptReply.Replica, tryPreAcceptReply.Instance)
		...
		case beacon := <-r.BeaconChan:
			//logger.Infof("Received Beacon from replica %d with timestamp %d", beacon.Rid, beacon.Timestamp)
...
func (r *Replica) bcastPreAccept(replica int32, instance int32, ballot int32, cmds []state.Command, seq int32, deps [DS]int32) {
	...
	for q := 0; q < r.N-1; q++ {
		...
		//logger.Infof("Sending to preAccept to replica = %d instance = %d.%d, thrifty = %t, n = %d", r.PreferredPeerOrder[q], r.Id, instance, r.Thrifty, n)
...
func (r *Replica)bcastCommit(replica int32, instance int32, cmds []state.Command, seq int32, deps [DS]int32) {
	...
	for q := 0; q < r.N-1; q++ {
		...
		// Start of change by xy
		if isUseBeacon {
			// This is to fix the bug that Epaxos execution stalls when beacon is
			// enabled for thrifty. The bug happens when PreAccept and CommitShort
			// requests go to different replicas if the preferred peer order changes
			// due to beacon results.
			if r.Thrifty {
...
func (r *Replica) handlePropose(propose *genericsmr.Propose) {
	...
	//logger.Infof("Starting instance %d.%d batching %d", r.Id, instNo, batchSize)
...
func (r *Replica) startPhase1(replica int32, instance int32, ballot int32, proposals []*genericsmr.Propose, cmds []state.Command, batchSize int) {
	...
	//logger.Infof("Bcsting preAccept for instance = %d.%d, cmdId = %d, cmdNum = %d, deps = %v", r.Id, instance, proposals[0].CommandId, len(cmds), deps)
	r.bcastPreAccept(r.Id, instance, ballot, cmds, seq, deps)

	cpcounter += batchSize

	if r.Id == 0 && DO_CHECKPOINTING && cpcounter >= CHECKPOINT_PERIOD {
		//logger.Infof("do checkpoint")
		cpcounter = 0
...
func (r *Replica) handlePreAcceptReply(pareply *epaxosproto.PreAcceptReply) {
	...
	if (r.N <= 3 && !r.Thrifty) || inst.lb.preAcceptOKs > 1 {
		...
		if !equal {
			//logger.Infof("preAcceptReply conflict for instance = %d.%d", pareply.Replica, pareply.Instance)
			conflicted++
		}
	}
	...
	if inst.lb.preAcceptOKs >= r.N/2 && inst.lb.allEqual && isInitialBallot(inst.ballot) {
		happy++
		//logger.Infof("preAcceptReply fast path for instance %d.%d", pareply.Replica, pareply.Instance)
		...
		//logger.Infof("preAcceptReply slow path for instance %d.%d", pareply.Replica, pareply.Instance)
...
func (r *Replica) handlePreAcceptOK(pareply *epaxosproto.PreAcceptOK) {
	...
	if inst.lb.preAcceptOKs >= r.N/2 && inst.lb.allEqual && isInitialBallot(inst.ballot) {
		...
		//logger.Infof("preAcceptOK slow path for instance %d.%d", r.Id, pareply.Instance)
		slow++
		inst.Status = epaxosproto.ACCEPTED
